...
class Detection:
    def __init__(
        self, preprocessing: Preprocessing, fuzzy_wuzzy_model: FuzzyWuzzyModel, cat_boost_model: CatBoostModel
    ):
        self.data = pd.read_csv("../data/database.csv")
        self.list_company_in_base = self.data.name_1.unique()
        self.preprocessing = preprocessing
        self.models: dict[str, FuzzyWuzzyModel] = {"fuzzy_wuzzy": fuzzy_wuzzy_model, "catboost": cat_boost_model}
